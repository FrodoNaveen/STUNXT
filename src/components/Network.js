    ...
    <div className="mainContainer bg-body-secondary card mt-5 phoneSize">
      ...
      <div className=" container bg-white pt-3 pb-2" style={{ boxShadow: "0px 15px 10px -15px #111" }}>
        <div className=" card-fluid d-flex justify-content-around text-center">
          {networkHeader.map((ele, index) => (
            <div key={index}>
              {selectNetworkHeader === ele ? <span className="cursorPointer" style={{ color: "#00d970" }} onClick={() => changeNetworkHeader(ele)}><b>{ele}</b></span>
                : <span className="cursorPointer" style={{ color: "#00000099" }} onClick={() => changeNetworkHeader(ele)}>{ele}</span>
              }
            </div>
          ))}
        </div>
      ...
      <div className="container mt-4">
        <div className="row ps-1">
          <div className="col-9">
            <input className="form-control rounded-4 borderWhite text-start" placeholder="Search by name" style={{ height: "60px", paddingRight: "20px" }} />
          </div>
          <div className="col-3">
            <div className="card rounded-4 borderWhite ms-3" style={{ height: "60px", width: "60px" }}>
              <img className="m-auto cursorPointer" src={refreshlogo} alt="filter logo" style={{ height: "24px", width: "24px" }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container scroll scrollHide" >
        <div className="b_100">
          {selectNetworkHeader === "Friends" ?
                    ...
                    <div className="pt-3 pb-3">
                      <div className="row">
                        ...
                        <div className="col-6 mt-2 text-end">
                          <button className="wishNowBtn"><small>Message</small></button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            </div> : selectNetworkHeader === "Requests" ? <RequestList /> : selectNetworkHeader === "Suggested" ? <SuggestList /> : <ContactList />}
        </div>
      </div>
      <Navbar index={index} />
    </div>
